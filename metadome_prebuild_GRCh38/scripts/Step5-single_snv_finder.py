...
def generate_variant_table(transcript_info):
    """
   This function takes a transcript's coding sequence and generates all possible SNVs,
    annotating each variant with its genomic position, reference and alternate bases,
    codon changes, and amino acid changes.
    ...
    for pos in range(len(transcript_seq)):
        ...
        for alt_base in [n for n in nucleotides if n != ref_base]:
            # Create variant sequence
            variant_seq = list(transcript_seq)
            variant_seq[pos] = alt_base
            ...
            var_codon = variant_seq[codon_start:codon_end]
            ...
            if var_codon in stop_codons:
                var_aa = 'X'
            else:
                var_aa = genetic_code.get(var_codon, 'X') if len(var_codon) == 3 else 'X'
            ...
            variants.append({
                'transcriptID': transcript_info.get('transcriptID', 'NA'),
                'chr': transcript_info.get('chromosome', 'NA'),
                'hg38': genomic_pos,
                'REF': ref_base if strand == '+' else str(Seq(ref_base).reverse_complement()),
                'ALT': alt_base if strand == '+' else str(Seq(alt_base).reverse_complement()),
                'strand': strand,
                'RefAA': ref_aa,
                'AAindex': codon_index + 1, # if strand == '+' else len(transcript_seq) // 3 - codon_index,
                'AltAA': var_aa,
...
def process_transcript(trn, transcripts_dict, output_dir):
    ...
    if transcript_info and transcript_info.get('coding_sequence'):
        variants = generate_variant_table(transcript_info)
