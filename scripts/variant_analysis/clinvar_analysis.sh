...
# homologous Pfam domains.
#
# Reads only files from the MetaDome data release, so the result can be
# reproduced from the Zenodo record with bcftools and bedtools alone. No
# database, no prebuild output and no running application are involved.
#
...
Optional:
  ...
  -s, --clnsig VALUE       ClinVar significance to select (default: Uncertain_significance)
...
echo "writing $FINAL ..."
{
  printf 'chrom\tpos\tclinvar_id\tref\talt\tclnsig\tclnrevstat\tclnhgvs\tgene_symbol\tuniprot_ac\tuniprot_pos\tpfam_id\tconsensus_pos\thomolog_missense_P_count\thomolog_missense_LP_count\tclinvar_P_accessions\tclinvar_LP_accessions\tmetadome_url\n'
  awk -F'\t' -v OFS='\t' '
      ...
      { k=$4 SUBSEP $16 SUBSEP $17 SUBSEP $18 SUBSEP $19
          ...
          print $1, $3, $4, $5, $6, $7, $8, $9,
              ...
              P[k], LP[k], PA[k], LPA[k], $20 }
  ' "$WORK/uniprot_to_symbol.tsv" "$WORK/with_evidence.tsv" "$WORK/in_domain.tsv" \
    | sort -k1,1 -k2,2n
} > "$FINAL"
