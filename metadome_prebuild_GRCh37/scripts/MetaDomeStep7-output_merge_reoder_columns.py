...
def main():
    ...
    metaposition = pd.merge(metaposition, pfam_names, on='PFAM_ID', how='left')
    del pfam_names
    ...
    final = pd.merge(merged, genomic, on='ENSEMBL_TR',how='inner')
    del merged, genomic
    
    final['uniprot_pos'] = final['uniprot_pos'] -1 
    
    final = pd.merge(final, metaposition, on=['ENSEMBL', 'uniprot_AA', 'uniprot_pos'], how='left')
    del metaposition
